...
logo = PhotoImage(file = "F:\CodeSoftTask\Password-Generator\icons\icono.png")
ventana.iconphoto(False, logo)
ventana.resizable(width=False, height=False)
ventana.configure(height=120, width=300)
...
tam_entry.place(anchor="nw", relx=0.28, rely=0.12, x=0, y=0)

generar_button = ttk.Button(ventana)
generar_button.configure(text='Gen password')
generar_button.place(anchor="nw", relx=0.36, rely=0.37, x=0, y=0)
generar_button.configure(command=generar_func)

caja_contra = ScrolledText(ventana)
caja_contra.place(anchor="nw",relheight=0.23,relwidth=0.68,relx=0.17,rely=0.68,x=0,y=0)
